...
$_lang["chunk_assigned_file"] = '对应的块元素文件';
$_lang["chunk_file_unusable_name"] = '此名称无法作为文件名';
$_lang["chunk_file_not_writable"] = '无法写入块元素文件 %s，因此未保存任何内容。请检查名称以及块元素目录的权限。';
$_lang["chunk_name_empty"] = '保存在文件中的块元素需要一个名称。';
$_lang["chunk_name_not_utf8"] = '此名称不是有效的 UTF-8，因此无法作为文件名。';
$_lang["chunk_name_not_nfc"] = '此名称使用了组合字符。请重新输入，否则 macOS 会将其视为与同一单词的另一种写法相同的文件。';
$_lang["chunk_name_too_long"] = '转义特殊字符后，此名称对于文件名来说过长。';
$_lang["chunk_name_collides"] = '此名称与块元素“%s”在 Windows 和 macOS 上会共用同一个文件，这两个系统仅按大小写区分名称。请重命名其中之一。';
