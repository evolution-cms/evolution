...
$_lang = [
  ...
  'cant_write_config_file_retry' => 'PHPが設定ファイルを読み書きできることを確認してください。推奨パーミッションは、ファイルが<code>0644 (rw-r--r--)</code>、<code>/core/config/database/connections/</code>ディレクトリが<code>0755 (rwxr-xr-x)</code>です。ファイルに実行権限は不要です。ホスティング事業者から明示的に求められない限り、<code>0777</code>は使用しないでください。その後、インストールを再試行してください。',
  ...
  'checking_if_database_config_writable' => '読み取り可能な設定ファイル<code>/core/config/database/connections/default.php</code>を作成または更新するための権限を確認: ',
  ...
  'status_failed_database_collation_does_not_match' => '問題があります - データベースの照合順序が一致していません（サーバーのデフォルトは %1$s です）。推奨: ホスティングパネルまたは phpMyAdmin でデータベースの照合順序を選択した UTF-8 照合順序に合わせ、もう一度確認してください。SET NAMES がサポートされている場合は続行できますが、この確認を通すためだけに %1$s へ変更しないでください。',
