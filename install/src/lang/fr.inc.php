...
$_lang = [
  ...
  'cant_write_config_file_retry' => 'Vérifiez que PHP peut lire et écrire le fichier de configuration. Les permissions recommandées sont <code>0644 (rw-r--r--)</code> pour le fichier et <code>0755 (rwxr-xr-x)</code> pour le répertoire <code>/core/config/database/connections/</code>. Le fichier ne nécessite pas de permission d’exécution. N’utilisez pas <code>0777</code>, sauf si votre hébergeur l’exige explicitement. Relancez ensuite l’installation.',
  ...
  'checking_if_database_config_writable' => 'Vérification des permissions nécessaires pour créer ou mettre à jour le fichier de configuration lisible <code>/core/config/database/connections/default.php</code>: ',
  ...
  'status_failed_database_collation_does_not_match' => 'échec - la collation de la base ne correspond pas (la valeur par défaut du serveur est %1$s). Recommandé : changez la collation de la base dans votre panneau d’hébergement/phpMyAdmin pour la faire correspondre à la collation UTF-8 choisie, puis relancez ce test. Si SET NAMES est pris en charge, vous pouvez continuer ; ne passez pas à %1$s uniquement pour valider ce test.',
