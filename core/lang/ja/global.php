...
$_lang["chunk_assigned_file"] = '対応するチャンクファイル';
$_lang["chunk_file_unusable_name"] = 'この名前はファイル名にできません';
$_lang["chunk_file_not_writable"] = 'チャンクファイル %s を書き込めなかったため、何も保存されませんでした。名前とチャンクディレクトリの権限を確認してください。';
$_lang["chunk_name_empty"] = 'ファイルに保存するチャンクには名前が必要です。';
$_lang["chunk_name_not_utf8"] = 'この名前は正しい UTF-8 ではないため、ファイル名にできません。';
$_lang["chunk_name_not_nfc"] = 'この名前は結合文字を使用しています。入力し直してください。そのままでは macOS が同じ語の別の表記と同一のファイルとして扱います。';
$_lang["chunk_name_too_long"] = 'この名前は特殊文字をエスケープするとファイル名として長すぎます。';
$_lang["chunk_name_collides"] = 'この名前とチャンク「%s」は Windows と macOS で同じファイルを共有してしまいます。これらは大文字と小文字だけで名前を区別します。どちらかの名前を変更してください。';
